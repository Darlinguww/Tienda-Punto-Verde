...
import { LayoutDashboard, Fan, Tv, WashingMachine, Refrigerator, Percent, Menu } from 'lucide-react';
...
const menuItems = [
  ...
  { id: 'Ventiladores', icon: Fan, label: 'Ventiladores' },
  { id: 'TV', icon: Tv, label: 'TV' },
  { id: 'Lavadoras', icon: WashingMachine, label: 'Lavadoras' },
  { id: 'Neveras', icon: Refrigerator, label: 'Neveras' },
  { id: 'Promociones', icon: Percent, label: 'Promociones' },
];
            ...
            {menuItems.map((item) => {
              ...
              return (
                <button
                  key={item.id}
                  onClick={() => {
                    onCategoryChange(item.id as Category);
